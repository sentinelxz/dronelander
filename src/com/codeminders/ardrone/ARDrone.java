...
import eu.hrasko.dronelander.DroneController;
import eu.hrasko.dronelander.Tachometer;
import eu.hrasko.p5video.VideoManager2;
...
public class ARDrone {
	...
	public enum Animation {
		PHI_M30_DEG(0), PHI_30_DEG(1), THETA_M30_DEG(2), THETA_30_DEG(3), THETA_20DEG_YAW_200DEG(4), THETA_20DEG_YAW_M200DEG(5), TURNAROUND(6), TURNAROUND_GODOWN(
				7), YAW_SHAKE(8), YAW_DANCE(9), PHI_DANCE(10), THETA_DANCE(11), VZ_DANCE(12), WAVE(13), PHI_THETA_MIXED(14), DOUBLE_PHI_THETA_MIXED(15), ANIM_MAYDAY(
				16);
	...
	public enum LED {
		BLINK_GREEN_RED(0), BLINK_GREEN(1), BLINK_RED(2), BLINK_ORANGE(3), SNAKE_GREEN_RED(4), FIRE(5), STANDARD(6), RED(7), GREEN(8), RED_SNAKE(9), BLANK(10), RIGHT_MISSILE(
				11), LEFT_MISSILE(12), DOUBLE_MISSILE(13), FRONT_LEFT_GREEN_OTHERS_RED(14), FRONT_RIGHT_GREEN_OTHERS_RED(15), REAR_RIGHT_GREEN_OTHERS_RED(16), REAR_LEFT_GREEN_OTHERS_RED(
				17), LEFT_GREEN_RIGHT_RED(18), LEFT_RED_RIGHT_GREEN(19), BLINK_STANDARD(20);
	...
	public enum ConfigOption {
		ACCS_OFFSET("control:accs_offset"), ACCS_GAINS("control:accs_gains"), GYROS_OFFSET("control:gyros_offset"), GYROS_GAINS("control:gyros_gains"), GYROS110_OFFSET(
				"control:gyros110_offset"), GYROS110_GAINS("control:gyros110_gains"), GYRO_OFFSET_THR_X("control:gyro_offset_thr_x"), GYRO_OFFSET_THR_Y(
				"control:gyro_offset_thr_y"), GYRO_OFFSET_THR_Z("control:gyro_offset_thr_z"), PWM_REF_GYROS("control:pwm_ref_gyros"), CONTROL_LEVEL(
				"control:control_level"), SHIELD_ENABLE("control:shield_enable"), EULER_ANGLE_MAX("control:euler_angle_max"), ALTITUDE_MAX(
				"control:altitude_max"), ALTITUDE_MIN("control:altitude_min"), CONTROL_TRIM_Z("control:control_trim_z"), CONTROL_IPHONE_TILT(
				"control:control_iphone_tilt"), CONTROL_VZ_MAX("control:control_vz_max"), CONTROL_YAW("control:control_yaw"), OUTDOOR("control:outdoor"), FLIGHT_WITHOUT_SHELL(
				"control:flight_without_shell"), BRUSHLESS("control:brushless"), AUTONOMOUS_FLIGHT("control:autonomous_flight"), MANUAL_TRIM(
				"control:manual_trim"), INDOOR_EULER_ANGLE_MAX("control:indoor_euler_angle_max"), INDOOR_CONTROL_VZ_MAX("control:indoor_control_vz_max"), INDOOR_CONTROL_YAW(
	...
	private void changeState(State newstate) throws IOException {
		...
		synchronized (state_mutex) {
			if (state != newstate) {
				log.severe("State changed from " + state + " to " + newstate);
				state = newstate;
	...
	public void connect(DataLogger videoLogger, DataLogger navdataLogger) throws IOException {
		try {
			...
			try {
				...
				log.log(Level.INFO, "Drone version string: " + versionStr);
				version = Integer.parseInt(versionStr.substring(0, versionStr.indexOf('.')));
			} catch (NumberFormatException e) {
				log.log(Level.INFO, "Failed to discover drone version. Using configuration for drone version: " + version, e);
			}
			...
			NavDataDecoder nav_data_decoder = (null == ext_nav_data_decoder) ? new ARDrone10NavDataDecoder(this, NAVDATA_BUFFER_SIZE) : ext_nav_data_decoder;

			ARDroneDataReader nav_data_reader = (null == navdataLogger) ? new LigthUDPDataReader(drone_addr, NAVDATA_PORT, navDataReconnectTimeout)
			...
			VideoDataDecoder video_data_decoder = (null == ext_video_data_decoder) ? getVideoDecoder(version) : ext_video_data_decoder;
			ARDroneDataReader video_data_reader = (null == videoLogger) ? getVideoReader(version) : new ARDroneDataReaderAndLogWrapper(new UDPDataReader(
					drone_addr, VIDEO_PORT, videoReconnectTimeout), videoLogger);
			...
			if (null != video_data_reader && null != video_data_decoder) {
				...
				boolean connected = ((VideoManager2) video_data_decoder).connect(VIDEO_PORT);
				cmd_queue.add(new ConfigureCommand("video:video_codec", "0x81"));
				new Thread(((VideoManager2) video_data_decoder)).start();
				if (connected) {
					System.out.println("VIDEO DRONE 2.0 connected");
				} else
					System.err.println("VIDEO DRONE 2.0 NOT connected");
			}
	...
	private VideoDataDecoder getVideoDecoder(int version) throws IOException {
		...
		case 2:
			return new VideoManager2(this, getInetAddress("192.168.1.1"));
	...
	private ARDroneDataReader getVideoReader(int version) throws IOException {
		...
		case 2:
			return null;// new TCPDataRader(drone_addr, VIDEO_PORT,
						// videoReconnectTimeout);
		default:
			return new LigthUDPDataReader(drone_addr, VIDEO_PORT, videoReconnectTimeout);
		}
	}
	...
	protected void navDataReceived(NavData nd) {
		...
		// System.out.println("Mode: " + nd.getMode());

		// BATTERY FOR MICHELLE
		try {
			java.nio.file.Files.write((new File("battery.txt")).toPath(), new Integer(nd.getBattery()).toString().getBytes("utf-8"), StandardOpenOption.CREATE,
	...
	public void selectVideoChannel(VideoChannel c) throws IOException {
		System.out.println("ARDrone.selectVideoChannel()");
		/*
	...
	public void takeOff() throws IOException {
		...
		if (!isEmergencyMode()) {
			...
			System.err.println("Cannot take off because in emergency mode!");
		}
	}
	...
	public void videoFrameReceived(int startX, int startY, int w, int h, int[] rgbArray, int offset, int scansize) {
		synchronized (image_listeners) {
			for (DroneVideoListener l : image_listeners)
				l.frameRecieved(startX, startY, w, h, rgbArray, offset, scansize);
		}
	}

	public void videoFrameReceived(BufferedImage im) {
		synchronized (image_listeners) {
			for (DroneVideoListener l : image_listeners)
				l.frameRecieved(im);
	 ...
	 * Read Drone version.
	 * 
	 * @return Drone version string e.g. "1.10.10". null - if version can't be
	 *         obtained
	 */
	...
	/**
	 * Reads drone configuration content. Please execute this method only when
	 * drone is connected, otherwise method will stick on waiting data from
	 * drone control port.
	 * 
	 * @return current values of Drone configuration, information about
	 *         software, motors, tilt limitation etc. Please see ARDrone
	 *         Developers guide (Section 8.1 Reading the drone configuration)
	 *         for full list of parameters. null if any error occurred
	 */
